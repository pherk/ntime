...
module namespace nicalfmt = "http://eNahar.org/exist/ns/lib/ical/format";
...
(:
    ...
    nicalfmt:format-dateTime(xs:dateTime(fn:current-dateTime()), "M-d-yy")
    => 8-27-10
     
    nicalfmt:format-dateTime(xs:dateTime(fn:current-dateTime()), "MM/dd/yyyy hh:mm:ss:SSSSS A")
    => 08/27/2010 01:31:30:00852 PM
     
    nicalfmt:format-dateTime(xs:dateTime("1345-08-01T16:47:49.619899-06:00"), "EEEE, MMMM dd, yyyy G @hh:m A")
    => Sunday, August 01, 1345 AD @04:47 PM
     
    nicalfmt:format-dateTime(xs:dateTime("-0044-05-13T00:00:00.00000-06:00"), "yyyy G")
    => 44 BC
     
:)
 
declare function nicalfmt:format-date($date as xs:date, $pattern as xs:string) {
    nicalfmt:format-dateTime(xs:dateTime($date), $pattern)
};
 
declare function nicalfmt:format-dateTime($date-time as xs:dateTime, $pattern as xs:string) {
 
    let $tokens := (
        let $first-pattern-char := fn:substring($pattern, 1, 1)
        let $remaining-pattern := fn:substring($pattern, 2)
        return nicalfmt:determine-token($remaining-pattern, $first-pattern-char, ())
    )
    ...
    let $formatted-tokens := (
        ...
        let $token-value := (
            ...
            then (nicalfmt:pad-to-fill(xs:string($month), 2, "0", fn:true()))
            else if ($token eq "MMM")
            then (nicalfmt:translate-month($month, fn:false()))
            else if ($token eq "MMMM")
            then (nicalfmt:translate-month($month, fn:true()))
            else if ($token eq "d")
            then ($day)
            else if ($token eq "dd")
            then (nicalfmt:pad-to-fill(xs:string($day), 2, "0", fn:true()))
            else if ($token eq "h")
            then (
                if ($hours > 12)
                then ($hours - 12)
            ...
            then (
                ...
                return nicalfmt:pad-to-fill(xs:string($under-13), 2, "0", fn:true())
            )
            else if ($token eq "H")
            then ($hours)
            else if ($token eq "HH")
            then (nicalfmt:pad-to-fill(xs:string($hours), 2, "0", fn:true()))
            else if ($token eq "m")
            then ($minutes)
            else if ($token eq "mm")
            then (nicalfmt:pad-to-fill(xs:string($minutes), 2, "0", fn:true()))
            else if ($token eq "s")
            then ($whole-seconds)
            else if ($token eq "ss")
            then (nicalfmt:pad-to-fill(xs:string($whole-seconds), 2, "0", fn:true()))
            ...
            then (
                ...
                return nicalfmt:pad-to-fill($rounded-millis, $num-places, "0", fn:true())
            )
            else if ($token eq "a")
            then (
                if ($hours > 11)
            ...
            then (nicalfmt:translate-day( nicalfmt:day-of-week($date-time), fn:false()))
            else if ($token eq "EEEE")
            then (nicalfmt:translate-day( nicalfmt:day-of-week($date-time), fn:true()))
            else if ($token eq "G")
            then (
                if ($year > 0)
                then ("AD")
...
declare function nicalfmt:translate-month($month-num as xs:int, $full as xs:boolean) {
    nicalfmt:translate-month($month-num, $full, "")
};
 
declare function nicalfmt:translate-month($month-num as xs:int, $full as xs:boolean, $lang as xs:string) {
    let $trans :=
        <month-translations>
...
declare function nicalfmt:translate-day($day-of-week-num as xs:int, $full as xs:boolean) {
    nicalfmt:translate-day($day-of-week-num, $full, "")
};
 
declare function nicalfmt:translate-day($day-of-week-num as xs:int, $full as xs:boolean, $lang as xs:string) {
    let $trans :=
        <day-translations>
...
declare function nicalfmt:day-of-week($date as xs:anyAtomicType?) as xs:integer? {
    if (fn:empty($date))
    then ()
    else (
...
declare function nicalfmt:determine-token($source, $current-tok, $tok-xml) {
    ...
    then (($tok-xml, <token>{$current-tok}</token>))
        ...
        return (
            ...
            then (
                ...
                return nicalfmt:determine-token($new-source, $new-tok, $tok-xml)
            )
            else (
                let $new-source := fn:substring($source, 2)
                let $new-current-tok := $first-seq-char
                let $new-tok-xml := ($tok-xml, <token>{$current-tok}</token>)
                return nicalfmt:determine-token($new-source, $new-current-tok, $new-tok-xml)
            )
        )
    )
};
 
declare function nicalfmt:pad-to-fill($value as xs:string, $length as xs:int, $pad-char as xs:string, $pad-left as xs:boolean) {
